...
function deepDup(arr) {
  if (Array.isArray(arr)) {
    // If the input is an array, create a new array for the copy.
    const copy = [];

    // Iterate through the elements of the input array.
    for (let i = 0; i < arr.length; i++) {
      // Recursively deep copy each element and add it to the copy array.
      copy[i] = deepDup(arr[i]);
    }

    return copy;
  } else {
    // If the input is not an array, it's a primitive value. Return it as-is.
    return arr;
  }
}
